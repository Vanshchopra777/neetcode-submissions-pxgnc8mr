...
public:
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& nums) {
        ...
        for(int i=0;i<n;i++){
           if(!pacific[i][0]) dfs(i,0,nums,pacific);
            if(!atlantic[i][m-1])dfs(i,m-1,nums,atlantic);
            
        }
        for(int j=0;j<m;j++){
           if(!pacific[0][j]) dfs(0,j,nums,pacific);
           if(!atlantic[n-1][j]) dfs(n-1,j,nums,atlantic);
        }
